...
以下两个案例均为浏览器里拖线搭建 → 点击调试 → DeepSeek 真实返回的结果，非演示摆拍。

### 案例 1：一句话冷笑话（3 节点 · 全绿成功）

`用户输入 → 大模型 → 结束`，大模型节点系统提示词：`你是一个冷笑话大师，只用一句话讲一个中文冷笑话，不要解释`

| 输入 | 大模型实际输出（586ms） |
|---|---|
| `程序员的笑话` | “程序员最讨厌的两件事：一是别人不写注释，二是自己写注释。” |

![冷笑话案例调试时间线](docs/assets/case-joke.png)

调试时间线逐节点展示状态 / 耗时 / 输入输出 JSON，画布节点同步亮起状态角标。

### 案例 2：新闻播报员（LLM + 音频链路，4 节点）

`用户输入 → 大模型 → 音频合成 → 结束`，系统提示词：`用一到两句话把用户输入改写成新闻播报口吻的短稿`

| 输入 | 大模型输出（716ms） | 音频节点 |
|---|---|---|
| `今天学校食堂推出了新菜糖醋排骨` | “今日，学校食堂推出全新菜品——糖醋排骨，为师生们带来更多美味选择。” | mock mp3 URL · 4s |

![新闻播报案例](docs/assets/case-tts.png)

> 💡 两个案例的大模型节点都**没填 API Key** —— 后端配了默认 Provider（`POST /api/v1/llm-providers`，`isDefault=true`）后所有 LLM 节点自动继承，团队共用一套 Key 很方便。
...
在节点配置面板填入 API Key，或给后端配默认 Provider（见上方案例说明）。这是最常见的第一步卡点，报错长这样：

![未配 Key 时的报错时间线](docs/assets/debug-timeline.png)
